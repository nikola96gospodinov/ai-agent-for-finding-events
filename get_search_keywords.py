...
def get_search_keywords(user_profile: UserProfile, model: BaseChatModel) -> List[str]:
    ...
    prompt_template = """
        ...
        Certain goals might benefit from including the user's age in the search queries. For example, "30s make new friends" and "40s dating" are good search queries. At the same time, "20s business networking" or "tech 40s" are not good search queries.
        
        Then move on to the user's interests. Don't include the user's age in the search queries for the interests.
        Some interests might be related to each other. For example, "tech" and "business" are related. "outdoors" and "hiking" are related. Make sure to combine interests that are related but don't force it. For example combining unrelated interests like "tech outdoors" and "motorsports hiking" don't make good search queries.
        ...
        Only include words like "networking" if they are extremely relevant to the rest of the search query but do not use excessively. 
        Don't use "networking" on its own or with the user's age.
